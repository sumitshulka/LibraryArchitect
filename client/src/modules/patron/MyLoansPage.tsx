...
import { AccountHeader, EmptyPanel, LoanRow, PatronError, PatronLoading, QuickLinks, SectionHeading, usePatronAccount } from "./patronShared";
...
export default function MyLoansPage() {
  ...
  return <MainLayout><div className="space-y-7"><AccountHeader eyebrow="My loans" title="Books currently with you" description="Due dates, pickup libraries, and any charges in one place." /><QuickLinks />
    ...
    <section><SectionHeading title={`${loans.length} active ${loans.length === 1 ? "loan" : "loans"}`} description="Your current checkouts" />{loans.length ? <div className="grid gap-3 md:grid-cols-2">{loans.map((loan) => <LoanRow key={loan.id} loan={loan} />)}</div> : <EmptyPanel icon={BookOpen} title="Your shelf is clear" description="You do not have any active loans right now." href="/catalog" />}</section>
